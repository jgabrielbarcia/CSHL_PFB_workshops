...
#  This script should be executed from unitigging/ecoli.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./ecoli.ovlStore.BUILDING/bucket$jobname ] ; then
  echo "Bucketizing job finished; directory './ecoli.ovlStore.BUILDING/bucket$jobname' exists."
  exit
fi
